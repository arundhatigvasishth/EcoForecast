...
async function start() {
    ...
    await Promise.race([
        ...
        new Promise((_, reject) =>
            setTimeout(
                () => reject(new Error("Mongo connect timeout (8s)")),
                8000
            )
        ),
    ]);
    ...
    app.post("/api/inputs", async (req: Request, res: Response) => {
        try {
            ...
            return res
                .status(201)
                .json({ ok: true, id: result.insertedId.toString() });
        } catch (err) {
            console.error(err);
            return res
                .status(500)
                .json({ ok: false, error: "Failed to save inputs" });
        }
    });
    ...
    app.get("/api/inputs/latest", async (_req: Request, res: Response) => {
        try {
            ...
            return res
                .status(500)
                .json({ ok: false, error: "Failed to fetch latest inputs" });
        }
    });
    ...
    app.get("/api/outputs/:inputId", async (req: Request, res: Response) => {
        try {
            ...
            if (!ObjectId.isValid(inputIdRaw)) {
                return res
                    .status(400)
                    .json({ ok: false, error: "Invalid inputId" });
            }
            ...
            if (!doc) {
                return res
                    .status(404)
                    .json({ ok: false, error: "Inputs not found" });
            }
            console.log("===== INPUT DOC USED FOR COMPUTE =====");
            ...
            const inputs = {
                electricity: [
                    {
                        q: "Q1",
                        usage: Number(q.q1.electricity.usage),
                        cost: Number(q.q1.electricity.amountPaid),
                    },
                    {
                        q: "Q2",
                        usage: Number(q.q2.electricity.usage),
                        cost: Number(q.q2.electricity.amountPaid),
                    },
                    {
                        q: "Q3",
                        usage: Number(q.q3.electricity.usage),
                        cost: Number(q.q3.electricity.amountPaid),
                    },
                    {
                        q: "Q4",
                        usage: Number(q.q4.electricity.usage),
                        cost: Number(q.q4.electricity.amountPaid),
                    },
                ],
                water: [
                    {
                        q: "Q1",
                        usage: Number(q.q1.water.usage),
                        cost: Number(q.q1.water.amountPaid),
                    },
                    {
                        q: "Q2",
                        usage: Number(q.q2.water.usage),
                        cost: Number(q.q2.water.amountPaid),
                    },
                    {
                        q: "Q3",
                        usage: Number(q.q3.water.usage),
                        cost: Number(q.q3.water.amountPaid),
                    },
                    {
                        q: "Q4",
                        usage: Number(q.q4.water.usage),
                        cost: Number(q.q4.water.amountPaid),
                    },
                ],
                fuel: [
                    {
                        q: "Q1",
                        usage: Number(q.q1.fuel.usage),
                        cost: Number(q.q1.fuel.amountPaid),
                    },
                    {
                        q: "Q2",
                        usage: Number(q.q2.fuel.usage),
                        cost: Number(q.q2.fuel.amountPaid),
                    },
                    {
                        q: "Q3",
                        usage: Number(q.q3.fuel.usage),
                        cost: Number(q.q3.fuel.amountPaid),
                    },
                    {
                        q: "Q4",
                        usage: Number(q.q4.fuel.usage),
                        cost: Number(q.q4.fuel.amountPaid),
                    },
                ],
            } as const;
            ...
            if (bad) {
                return res.status(400).json({
                    ok: false,
                    error: "Invalid usage/amountPaid values",
                });
            }

            // 5) Compute
            const setupAndMaint = calcSetup.calculateSetupAndMaintenance(
                inputs as any
            );
            const outputs = calcOutputs.calcDeterministicOutputs(
                inputs as any,
                setupAndMaint as any,
                20,
                0.02
            );
            console.log("===== COMPUTED YEAR 1 BASELINE TOTAL =====");
            console.log(outputs.year1.baseline.total);
            console.log("===========================================");
